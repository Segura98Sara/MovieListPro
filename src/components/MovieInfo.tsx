...
export function MovieInfo({ movie }: Props) {
  return (
    <article className='text-start p-1 text-gray-200'>
      <header>
      ...
      <main className='space-y-4'>
        ...
        <ul className='flex gap-2 justify-between'>
          <li className='text-center p-2 flex-grow shadow bg-teal-950 rounded'>
            <legend className='font-semibold'>Category</legend>
            <a
              href={movie.category}
              title={movie.category}
              className='hover:underline line-clamp-1 text-gray-300 text-base'
            >
              {movie.category}
            </a>
          </li>
